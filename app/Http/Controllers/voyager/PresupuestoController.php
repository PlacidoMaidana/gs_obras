...
class PresupuestoController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function ModalItem_elegir()
    {
        return  datatables()->of(DB::table('items')
        ->join('categoria_items', 'items.COD_CATEGORIA', '=', 'categoria_items.id')
        ->select(            
            'items.id',
            'items.nombre',
            'categoria_items.descripcion as categoria',
            'items.DESCRIPCION as descripcion',
            'items.UNIDAD as unidad',
            'items.BASE_PRECIO as Base_Precio',
